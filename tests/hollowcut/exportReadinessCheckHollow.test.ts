...
describe("hollow.hollowcut.export_readiness_check", () => {
  ...
  it("V1 catalog remains exactly 12 (protected); Hollowcut catalog separate at 8 including export_readiness_check", () => {
  ...
  it("V1 remains exactly 12; Hollowcut remains exactly 9; no export/render/FFmpeg/media mutation behavior added", () => {
    expect(V1_HOLLOW_MANIFESTS).toHaveLength(12);
    expect(HOLLOWCUT_HOLLOW_MANIFESTS.length).toBe(9);
    expect(HOLLOWCUT_HOLLOW_MANIFESTS.map((m: any) => m.hollow_id)).toContain("hollow.hollowcut.export_plan_preview");
